...
namespace MicahW.PointGrass {
    ...
    public class PointGrassRendererEditor : Editor {
        ...
        private void OnEnable() {
            // Play-mode enter/exit rebuilds this inspector after the inspected
            // instance is already destroyed. serializedObject then throws
        ...
        private bool HasLiveTargets() {
            UnityEngine.Object[] inspected = targets;
            if (inspected == null || inspected.Length == 0) return false;
            for (int i = 0; i < inspected.Length; i++) {
                if (inspected[i] == null) return false;
            }
            return true;
        }
        ...
        public override void OnInspectorGUI() {
            if (!HasLiveTargets()) return;
            if (prop_distSource == null) BindProperties();
